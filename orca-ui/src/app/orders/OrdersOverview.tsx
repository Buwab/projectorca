...
export default function OrdersOverview({ orders: initialOrders }: { orders: Order[] }) {
  ...
  const groupedProductsByDate = (products: Product[]) => {
    ...
    products.forEach((p) => {
      // Find matching order line using order_line_id (much more reliable)
      const matchingOrderLine = orderLines.find(line => 
        line.id === p.order_line_id
      );
  ...
  Object.entries(groupedProductsByDate(selectedOrder.parsed_data.products)).map(
    ([date, products]) => (
      <div key={date} className="mb-3">
        ...
        {products.map((p, i) => {
          // Use id as the unique key for this specific product
          ...
          return (
            <div key={i} className="flex justify-between text-sm border-b py-1">
              ...
              <span>
                {p.quantity} {p.unit}
                {(() => {
                  // Find matching order line to get delivery date from database
                  const matchingOrderLine = orderLines.find(line => 
                    line.id === p.order_line_id
                  );
                  const deliveryDate = matchingOrderLine?.delivery_date;
